...
### PLAID Account Transfer Handling (Updated 2025-12-22)

**Strict PLAID Categorization:**

- `TRANSFER_IN_ACCOUNT_TRANSFER` → `income/account_transfer` (weight=1.0, confidence=0.98)
- `TRANSFER_OUT_ACCOUNT_TRANSFER` → `expense/account_transfer` (weight=1.0, confidence=0.98)

**Why the Change:**
Previously, these were categorized as `transfer/internal` or `transfer/external` with weight=0.0 (excluded from calculations). This led to:

- Understatement of actual cash inflows/outflows
- Inflated disposable income calculations
- Missed account-to-account payment patterns

**New Behavior:**

- Strict PLAID matching takes precedence over keyword matching
- Account transfers are included in income/expense totals at 100% weight
- Keyword-based internal transfers (savings pots, etc.) remain excluded via `transfer/internal`

### Transfer Keywords (Neobanks & Internal - Keyword-Based Exclusion)
